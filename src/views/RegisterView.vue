...
onMounted(() => {
  axios.post(apiEndpoint + '/register', {
    ...
    console.log(err)
    console.log(apiEndpoint+'/register')
  })
});

// function registerUser(): void {
//   axios
//     .post<Token[]>(apiEndpoint + '/register', { username: username.value, email: email.value, password: password.value, firstName: firstName.value, lastName: lastName.value })
//     .then((res) => {
//       email.value = ''
//       username.value = ''
//       firstName.value = ''
      ...
      <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
          ...
          <form class="space-y-4 md:space-y-6" @submit.prevent="registerUser">
            ...
            <button type="submit" :disabled="error" class="w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 disabled:opacity-50">Create an account</button>
